...
def generate_speed_profile(path_x, path_y, max_v=20.0, max_lat_acc=2.0, max_long_acc=1.0, max_long_dec=1.0):
    dx = np.diff(path_x)
    dy = np.diff(path_y)
    ds_list = np.sqrt(dx**2 + dy**2)
    ds_padded = np.append(ds_list, ds_list[-1]) 
    
    xp = np.gradient(path_x) / ds_padded
    yp = np.gradient(path_y) / ds_padded
    xpp = np.gradient(xp) / ds_padded
    ypp = np.gradient(yp) / ds_padded
    ...
    # 公式: v = sqrt(a_lat / |kappa|) ，a_lat = v^2/R = v^2 * kappa
    eps = 1e-6
    v_limit = np.sqrt(max_lat_acc / (curvature + eps))
    v_ref = np.minimum(max_v, v_limit)
    # [end] TODO 3.1.b
    ...
    # 限制啟動速度
    v_ref[0] = min(v_ref[0], 5.0)
    # 限制終點速度 (停止)
    v_ref[-1] = 0.0

    # 1. Forward Pass: 限制加速度 (Acceleration Limit)
    # 公式: vi^2 <= vi-1^2 + 2 * a_acc * ds
    for i in range(1, n):
        v_max_fwd = np.sqrt(v_ref[i-1]**2 + 2 * max_long_acc * ds_list[i-1])
        v_ref[i] = min(v_ref[i], v_max_fwd)

    # 2. Backward Pass: 限制減速 (Braking Limit / Deceleration)
    # 公式: vi^2 <= vi+1^2 + 2 * a_dec * ds
    for i in range(n-2, -1, -1):
        v_max_bwd = np.sqrt(v_ref[i+1]**2 + 2 * max_long_dec * ds_list[i])
        v_ref[i] = min(v_ref[i], v_max_bwd)
