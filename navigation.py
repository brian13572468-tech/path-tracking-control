    ...
    while(True):
        ...
        if path is not None and collision_count == 0:
            if args.simulator == "basic":
                ...
                # 取得模擬器中定義的半軸距 l
                l = simulator.l
                # 取得輪子半徑：根據模擬器的 init，半徑是 wu 的一半
                r = simulator.wu / 2.0
                
                # 將角速度從 deg/s 轉為 rad/s 進行物理計算
                w_rad = np.deg2rad(next_w)
                
                # 逆向運動學公式 (Inverse Kinematics)
                # v = (r/2)(wr + wl)
                # w = (r/2l)(wr - wl)
                # 反解得到：
                phi_left_rad = (next_v - w_rad * l) / r
                phi_right_rad = (next_v + w_rad * l) / r
                
                # 轉回 deg/s 以符合 ControlState 的輸入要求
                next_lw = np.rad2deg(phi_left_rad)
                next_rw = np.rad2deg(phi_right_rad)
                # [end] TODO 2.2.2
